...
void raspi_machine_init(MachineState *machine)
{
    ...
    /* Mount my-led device */
    printf("\n[RASPI] Mounting my-led at 0x3c000000\n");
    DeviceState *led_dev = qdev_new(TYPE_MY_LED);
    sysbus_realize_and_unref(SYS_BUS_DEVICE(led_dev), &error_fatal);
    sysbus_mmio_map(SYS_BUS_DEVICE(led_dev), 0, 0x3c000000);
    printf("[RASPI] my-led mapped at 0x3c000000\n"); fflush(stdout);
}
